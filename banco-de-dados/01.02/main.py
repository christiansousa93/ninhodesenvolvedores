...
try:
    ...
    funcionarioEscolhido = input(
        "Escolha o id do funcionario que deseja alterar: ")

    funcionarioConsulta = con.consultarBanco(f'''
    SELECT * FROM "Funcionarios"
    WHERE "ID" = '{funcionarioEscolhido}'
    ''')
    funcionario = Funcionario(funcionarioConsulta[0][0], funcionarioConsulta[0][1],
                              funcionarioConsulta[0][2], funcionarioConsulta[0][3], funcionarioConsulta[0][4])

    print("Funcionario escolhido foi:")
    print(funcionario.imprimirFuncionario())

    opcoes = input(
        "Você deseja alterar as informações basicas(1) ou o departamento(2)?")
    ...
    match opcoes:
        case "1":
            ...
            con.manipularBanco(
                funcionario.atualizarFuncionario("Funcionarios"))
        case "2":
            funcionario.idDepartamento = input(
                "Insira o id do novo Departamento")
            con.manipularBanco(
                funcionario.atualizarDepartamentoFuncionario("Funcionarios"))
